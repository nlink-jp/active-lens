...
// jsonSession is one unbroken stretch of work. Under day_boundary "session" it
// is never split at a day boundary; under "strict" a boundary ends it, and the
// carried flags say which of its ends is a cut rather than a real one.
type jsonSession struct {
	StartUnix        int64       `json:"start_unix"`
	EndUnix          int64       `json:"end_unix"`
	Start            string      `json:"start"`
	End              string      `json:"end"`
	CarriedIn        bool        `json:"carried_in"`  // start is a boundary cut
	CarriedOut       bool        `json:"carried_out"` // end is a boundary cut
	OperatingSeconds int64       `json:"operating_seconds"`
	PresentSeconds   int64       `json:"present_seconds"`
...
type jsonDayTimeline struct {
	...
	CarriedIn        bool          `json:"carried_in"`  // work_start is this day's boundary
	CarriedOut       bool          `json:"carried_out"` // work ran past the next boundary
	OperatingSeconds int64         `json:"operating_seconds"`
	PresentSeconds   int64         `json:"present_seconds"`
...
func printTimelineHuman(w io.Writer, tl jsonTimeline, loc *time.Location) {
	...
	for _, d := range tl.Days {
		...
		// A carried end is a cut, not a real start or finish. Say so on the line
		// that shows it, or the boundary reads as a suspiciously punctual workday.
		if d.CarriedIn {
			fmt.Fprint(w, "   · continues from previous day")
		}
		if d.CarriedOut {
			fmt.Fprint(w, "   · continues into next day")
		}
		fmt.Fprintln(w)
		for _, b := range d.Breaks {
