...
### 日の区切りが自分のものでないとき（`work.day_boundary`）

上の規則は個人の記録としては正しいものです。22:00 から翌 09:00 までの作業は一晩の
仕事であり、始めた夜へ帰属します。しかし勤務日が他者によって定義されている場合——
一日が 05:00 に始まる勤務先——には誤りになります。05:00 以降の時間が、規則の上では
既に終わっていた日へマージされてしまうからです。

`work.day_boundary` はこの 2 つの読み方を選びます。

| 値 | 境界を貫いたセッション |
|----|------------------------|
| `session`（既定） | 丸ごと保たれ、開始した日へ帰属する |
| `strict` | 境界で切られ、各日にはその日に収まった分だけが計上される |

`strict` では勤務ログと合計台帳が日ごとに一致し、メニューバーの数字は境界で振り出しに
戻り、境界の時点で既に作業中だった日には `continues from previous day`
（JSON では `carried_in`）が付きます。この印には意味があります——そういう日の
`work_start` は境界そのものであって、誰かが実際に机に向かった時刻ではないからです。

どちらのモードも同じ生サンプルを読みます。閾値と境界は導出時に適用されるので、設定を
切り替えれば過去の履歴すべてが読み直され、移行も欠落も発生しません。
[ADR 0002](docs/ja/adr/0002-configurable-day-boundary.ja.md) を参照してください。
...
`carried_in` / `carried_out` が含まれます。`--since` の日付を自分で計算するより
...
これは稼働ログと合計台帳の差です。`work.day_boundary = "strict"` にすると、セッションも
境界で切られるのでこの差は無くなります。
...
| `work.day_boundary` | `"session"` | `"session"`: セッションは開始した日に留まる。`"strict"`: 境界で切られ、各日にその日の分だけが計上される |
